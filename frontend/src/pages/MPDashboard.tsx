...
export const MPDashboard: React.FC = () => {
  ...
  const mpId = scope.mp_id || 1;

  const { data: summary, loading: summaryLoading } = useDashboardSummary({ mp_id: mpId });
  const { data: rankedWorks, loading: rankedLoading } = useRiskRanked(10, mpId);
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        <div>
          ...
          <h1 className="text-2xl font-black tracking-tight text-slate-900 mt-2">
            Member of Parliament Portal (MP #{mpId})
          </h1>
          <p className="text-xs text-slate-500 mt-0.5">
            Constituency Project Integrity & Compliance Monitoring
          </p>
        </div>

        {/* KPIs & Annual Cap Tracking */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm hover:shadow-xs border-l-4 border-l-blue-500 transition-all duration-200 group">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold uppercase tracking-wider text-slate-500">Constituency Works</span>
            ...
            <div className="text-[11px] font-medium text-slate-400 mt-1">Recommended by MP #{mpId}</div>
          </div>
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm hover:shadow-xs border-l-4 border-l-rose-500 transition-all duration-200 group">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold uppercase tracking-wider text-slate-500">Audit Flags in Constituency</span>
            ...
            <div className="text-[11px] font-medium text-rose-500 mt-1">Requires clarification</div>
          </div>
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm hover:shadow-xs border-l-4 border-l-emerald-500 transition-all duration-200 group">
            ...
            <div className="text-[11px] font-medium text-slate-400 mt-1">Released funds utilised</div>
          </div>

          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm hover:shadow-xs border-l-4 border-l-indigo-500 transition-all duration-200 group">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold uppercase tracking-wider text-slate-500">₹5 Crore Annual Cap</span>
            ...
            <div className="text-2xl font-black tracking-tight text-indigo-600 font-mono mt-2">
              Compliant
            </div>
            <div className="text-[11px] font-medium text-indigo-500 mt-1">No annual ceiling breaches</div>
          </div>
        </div>

        {/* Priority Works for MP */}
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between">
            <div>
              <h2 className="text-sm font-bold text-slate-900">Constituency Works Requiring Attention</h2>
              <p className="text-xs text-slate-400">Projects with elevated risk indicators or delay flags</p>
            </div>
          </div>
        ...
        <RiskWorklist
          ...
          title={`Constituency Works Ledger (MP #${mpId})`}
          subtitle="Constituency project integrity, compliance monitoring, and anomaly ranking"
        />
      </main>
